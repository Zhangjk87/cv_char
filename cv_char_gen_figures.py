...
def main():
    ...
    frequency = []
    ideal_forward_slope = []
    ideal_forward_y_intercept = []
    ideal_forward_x_intercept = []
    ideal_reverse_slope = []
    ideal_reverse_y_intercept = []
    ideal_reverse_x_intercept = []

    csv_name = 'line_params.csv'
    with open(os.path.join(dir, csv_name), 'r') as csvfile:
        ...
        for row in reader:
            ...
            ideal_forward_slope.append(float(row['Ideal Forward Slope']))
            ideal_forward_y_intercept.append(float(row['Ideal Forward Y Intercept']))
            ideal_forward_x_intercept.append(float(row['Ideal Forward X Intercept']))
            ideal_reverse_x_intercept.append(float(row['Ideal Reverse Slope']))
            ideal_reverse_slope.append(float(row['Ideal Reverse Y Intercept']))
            ideal_reverse_y_intercept.append(float(row['Ideal Reverse X Intercept']))
    ...
    for file_num in range(len(temperature)):
        ...
        with open(os.path.join(dir, csv_name), 'w', newline='') as csvfile:
            ...
            ax.plot(x_forward, ideal_forward_slope[file_num] * x_forward + ideal_forward_y_intercept[file_num], '-', label='Forward Sweep Ideal Line')
            ax.plot(x_reverse, ideal_reverse_slope[file_num] * x_reverse + ideal_reverse_y_intercept[file_num], '-', label='Reverse Sweep Ideal Line')
            ...
            averaged_slope = np.mean((ideal_forward_slope[file_num], ideal_reverse_slope[file_num]))
            averaged_x_intercept = np.mean((ideal_forward_x_intercept[file_num], ideal_reverse_x_intercept[file_num]))
