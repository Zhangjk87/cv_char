...
#         yield find_streak(cap_vs_volt, start_index, diff_threshold)
...
def find_idealCV_line(cap_vs_volt):
    ...
    longest_streak_slope, longest_streak_dist, longest_streak_x_intercept = 0,0,0
    for start_index in range(cap_vs_volt.shape[0]):
        current_streak_size, current_streak_dist, current_slope, current_x_intercept = find_streak(cap_vs_volt, start_index, diff_threshold)
...
def main(argv):
    ...
    for file_number in range(310, 315, 5):
        ...
        fig, ax = plt.subplots()
        axes = plt.gca()
        axes.set_ylim([.99*np.min(cap_vs_volt_forward[:,1]), 1.01*np.max(cap_vs_volt_forward[:,1])])
